...
class AdminController extends EAdminController
{
    ...
    public function beforeAction($action)
    {
        ...
        $categoriesMenu = array(
            array(
                'label' => 'Без раздела',
                'url' => array('/faq/admin/index', 'catId' => 0),
            )
        );
    ...
    {
        $this->pageTitle = 'Вопрос-ответ - разделы';
		$this->clips['content_title']= 'Вопрос-ответ - разделы';
		...
		if (isset($_GET['catId'])) {
            ...
            $dataProvider = new CActiveDataProvider(Faq::model()->waiting());
            $this->breadcrumbs = array(
                'Вопрос-ответ',
            );
        }
        ...
        if (isset($_POST['Faq'])) {
            foreach ($dataProvider->data as $i=>$item) {
                if (isset($_POST['Faq'][$item->id]['status'])) {
                    $item->status = $_POST['Faq'][$item->id]['status'];
                    if (isset($_POST['Faq'][$item->id]['weight'])) {
                        $item->weight = $_POST['Faq'][$item->id]['weight'];
                        $item->visible = $_POST['Faq'][$item->id]['visible'];
                    }
                    $item->save();
                }
            }
    ...
    public function actionCreate()
    {
        ...
        if (isset($_POST['Faq'])) {
            ...
            if ($model->save()) {
			    Yii::app()->user->setFlash('success','Вопрос создан успешно.');
    ...
    public function actionUpdate()
    {
		...
		$this->clips['content_title']= 'Редактирование вопроса';
		if (!isset($_GET['id']))
            die();

        $model = Faq::model()->findByPk($_GET['id']);
        if (isset($_POST['Faq'])) {
    ...
    public function actionDelete()
    {
        ...
        if ($model !== null) {
            ...
            if ($model->delete()) {
                Yii::app()->user->setFlash('success', 'Вопрос удален успешно.');
			}
        }

...
class NestedSetBehavior extends CActiveRecordBehavior
{
	...
	public function getParent()
	{
		...
		$criteria=array(
			'condition'=>$alias.'.'.$db->quoteColumnName($this->leftAttribute).'<'.$owner->{$this->leftAttribute}
				//' AND '.$alias.'.'.$db->quoteColumnName($this->rightAttribute).'>'.$owner->{$this->rightAttribute},
				.' AND '.$alias.'.'.$db->quoteColumnName($this->levelAttribute).'='.($owner->{$this->levelAttribute}-1),
			'order'=>$alias.'.'.$db->quoteColumnName($this->rightAttribute) . ' DESC',
		);
    ...
    public function hierarchical($tree = array()) {
        ...
        foreach($tree as $k=>$node){
            if($node->level == 1) {
                ...
                $tmp[0] = $node;//$tmp[0] = главная
                $parents[1] = & $node->childs;//$parents[$node->level] = & $tmp[$k]->childs;//$parents[1] = $node->childs;
            } else {
        ...
        if($tmp[0]->childs){
            array_unshift($tmp[0]->childs, $tmp[0]);
            return $tmp[0]->childs;
        }else{
            return $tmp;
        }
